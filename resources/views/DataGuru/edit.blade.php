    ...
    <div class="container mt-4">
        <div class="card shadow rounded">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('dataguru.update', $guru->nip) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="nip" id="nip"
                                ...
                                value="{{ old('nip', $guru->nip) }}" pattern="\d{18}"
                                title="NIP harus terdiri dari 18 digit angka" required maxlength="18" inputmode="numeric">
                            @error('nip')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="username_user" id="username_user"
                                class="form-select @error('username_user') is-invalid @enderror" required>
                                <option value="">-- Pilih User --</option>
                                @foreach ($users as $user)
                                    <option value="{{ $user->username }}"
                                        {{ old('username_user', $guru->username_user) == $user->username ? 'selected' : '' }}>
                                        {{ $user->username }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="kode_mapel" class="form-select @error('kode_mapel') is-invalid @enderror"
                                required>
                                <option value="">-- Pilih Kode Mata Pelajaran --</option>
                                @foreach ($mataPelajaran as $mapel)
                                    <option value="{{ $mapel->kode }}"
                                        {{ old('kode_mapel', $guru->kode_mapel) == $mapel->kode ? 'selected' : '' }}>
                                        {{ $mapel->kode }}
                                    </option>
                                @endforeach
                            </select>
